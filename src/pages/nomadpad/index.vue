                    ...
                    <Step> 
                      <template slot="title"> 
                        <div class="fcsb-container"> 
                          ...
                          <span v-if="currentStep >= 1 && userRegistered" class="status-label success font-small weight-bold" 
                            >Registered</span 
                          > 
                          <span v-if="currentStep > 1 && !userRegistered" class="status-label failed font-small weight-bold" 
                            >Not Registered</span 
                          > 
                        </div> 
          ...
          <div class="project-detail-container"> 
            ...
            <div class="project-detail-condition"> 
              ...
              > 
                ...
                <span class="font-medium"> 
                  You can pay and claim once the whitelisting ends. 
                  Keep in touch. 
                </span> 
              ...
              <div v-else-if="currentStep === 2 && wallet.connected" class="project-detail-item"> 
                <div v-if="currentTimestamp < fertilizer.sales_start_date" class="project-detail-sales"> 
                  ...
                  <div 
                    v-else-if="((!currentStatus.win) || !userRegistered)" 
                    class="text-center" 
                  > 
                ...
                <div 
                  v-else-if=" 
                    currentTimestamp > fertilizer.sales_start_date && currentTimestamp < fertilizer.sales_end_date && wallet.connected 
                  " 
                > 
                  <div class="project-detail-open"> 
                    <div v-if="userRegistered"> 
                      ...
                      <div v-if="KYCStatus.userVerified && KYCStatus.step === 3" class="buy-form"> 
                        ...
                        <div class="receive-amount"> 
                          ...
                          <div class="receive-amount-output fcs-container"> 
                            <img class="coin-icon" :src="fertilizer.logo" /> 
                            <span class="receive-amount-value font-medium weight-semi spacing-small" 
                              >{{fertilizer.ido_info.sale_rate ? (buyAmount / fertilizer.ido_info.sale_rate).toFixed(fertilizer.decimals_mint) : ''}} {{ fertilizer.token_info.symbol }}</span 
                            > 
                          </div> 
                        </div> 
                        <div class="btn-container"> 
                    ...
                    <div 
                      v-else-if="(!currentStatus.win || !currentStatus.subscribe) && false" 
                      class="text-center" 
                    > 
                      <div class="fcc-container mb-8"> 
                        <img class="alert-icon" src="@/assets/icons/alert-white.svg" /> 
                  ...
                  <div class="project-detail-open"> 
                    ...
                    <div class="buy-form"> 
                      <div class="token-amount fcsb-container"> 
                        <div class="token-amount-input fcs-container"> 
                          <CoinIcon class="coin-icon" :mint-address="fertilizer.mint" /> 
                          <input class="font-medium weight-bold" type="number" placeholder="0"  v-model="userPaidAmount" disabled /> 
                        </div> 
                        <span class="font-xsmall weight-semi token-max-amount" 
                      ...
                      <div class="receive-amount"> 
                        ...
                        <div class="receive-amount-output fcs-container"> 
                          <img class="coin-icon" :src="fertilizer.logo" /> 
                          <span class="receive-amount-value font-medium weight-semi spacing-small" 
                            >{{fertilizer.ido_info.sale_rate ? (userPaidAmount / fertilizer.ido_info.sale_rate).toFixed(fertilizer.decimals_mint) : ''}} {{ fertilizer.token_info.symbol }}</span 
                          > 
                        </div> 
                      </div> 
              ...
              <div 
                v-else-if="currentStep === 3 && currentTimestamp > fertilizer.distribution_start_date && wallet.connected" 
                class="project-detail-item" 
              > 
                <div 
                ...
                > 
                  ...
                  <div class="buy-form"> 
                    <div class="token-amount fcsb-container"> 
                      <div class="token-amount-input fcs-container"> 
                        <CoinIcon class="coin-icon" :mint-address="fertilizer.mint" /> 
                        <input class="font-medium weight-bold" type="number" placeholder="input amount" v-model="userPaidAmount" disabled /> 
                      </div> 
                      <span class="font-xsmall weight-semi token-max-amount" 
                        > {{ fertilizer.price_token }}</span 
                      > 
...
export default Vue.extend({ 
  ...
  methods: { 
    ...
    async getProjectData() { 
      ...
      for (const item of responseData) { 
        let project = { 
          ...
          short_desc: 'NeoNomad provides an all-in-one integrated ecosystem, including a decentralized exchange, integrated payment services, a Launchpad, DeFi products and an NFT marketplace to make decentralized finance accessible to all.', 
          slug: 'neo_namad', 
          hard_cap: '198K', 
          subscribers: 0, 
    ...
    goBack() { 
      ...
      window.location.href = "https://www.neonomad.finance/" 
      // this.$router.push({ 
      //   path: 'https://neonomad.finance' 
      // }) 
    }, 
    ...
    async onClaimTokens() { 
      ...
      if(this.wallet.connected){ 
        let projectTokenAccount = get( 
          this.wallet.tokenAccounts, 
          `${this.fertilizer.mint}.tokenAccountAddress` 
        ) 
      if(projectTokenAccount){ 
        projectTokenAccount = new PublicKey(projectTokenAccount) 
      } 
      else { 
        projectTokenAccount = await checkWalletATA(this.fertilizer.mint, LaunchpadProgram.provider.connection, LaunchpadProgram.provider.wallet.publicKey) 
      } 
        let res = await claimTokens(new PublicKey(this.fertilizer.mint), projectTokenAccount) 
        if (res.success) { 
          const amount = (res.amount / Math.pow(10, this.fertilizer.decimals_mint)).toFixed(this.fertilizer.decimals_mint) 
          this.$notify.success({ 
    ...
    async onClickBuyNow() { 
      ...
      if(priceTokenAccount){ 
        ...
        priceTokenAccount = await checkWalletATA(this.fertilizer.price_token_mint, LaunchpadProgram.provider.connection, LaunchpadProgram.provider.wallet.publicKey) 
      } 
      const decimals_sale = await getMintDecimals(LaunchpadProgram.provider.connection, new PublicKey(this.fertilizer.price_token_mint)) 
      const amp_sale = Math.pow(10, decimals_sale) 
    ...
    async contextualizeUser() { 
      ...
      try { 
        ...
        if(userData) { 
          ...
          this.userPaidAmount = ((userData.paidAmount.toNumber() - userData.claimedAmount.toNumber()) / Math.pow(10, this.fertilizer.decimals_sale)).toFixed(this.fertilizer.decimals_sale) 
          this.userAllocation = ((userData.paidAmount.toNumber() - userData.claimedAmount.toNumber()) / Math.pow(10, this.fertilizer.decimals_sale) / this.fertilizer.ido_info.sale_rate).toFixed(this.fertilizer.decimals_mint) 
        } 
        else { 
